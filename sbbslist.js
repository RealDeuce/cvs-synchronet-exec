// $Id: sbbslist.js,v 1.16 2017/12/09 12:02:26 rswindell Exp $
    ...
    for(i in bbs.service) {
        ...
        if(result == false) {
            ...
            if(result.hello.length && result.hello[0].indexOf("Synchronet BBS") == 0) {
                ...
                bbs.preview = graphic.base64_encode(72);
                return true;
            }
...
function add_entry(list)
{
	...
	var host_name = getstr("Host name or IP address", client.host_name);
...
function edit(bbs)
{
	...
	while(js.global.bbs.online) {
		...
		switch(which) {
			case 1:
				var value = getstr("Since (YYYY-MM-DD)");
				if(value)
